...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(_.toLower).map {case (c, cs) => (c, cs.length)}.toList.sortBy(_._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    s.flatMap(wordOccurrences).groupBy(_._1).map{ case (c, ocs) => (c, ocs.map(_._2).sum)}.toList.sortBy(_._1)
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary.map(w => (wordOccurrences(w), w)).groupBy(_._1).map{ case (ocs, ocs_words) => (ocs, ocs_words.map(_._2))}
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
      ...
      case (char, count) :: rest =>
        (for {
          n <- 0 to count
          o <- combinations(rest)
        } yield if(n == 0) o else (char, n) :: o).toList
  }
