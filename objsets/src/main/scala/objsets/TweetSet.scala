...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /**
   * def merge(set, elem): TweetSet = {
   *   if that contains elem then that
   *   if that not contains elem then that.incl(elem)
   * }
   *
   * val merged = merge(that, elem)
   * val leftMerged = left.union(merged)
   * val rightMerged = right.union(leftMerged)
   * rightMerged
   *
   * @param that
   * @return
  ...
  override def mostRetweeted: Tweet = {
    def max(x: Tweet, tweet: TweetSet): Tweet = {
      try {
        val y = tweet.mostRetweeted
        if (x.retweets > y.retweets) x else y
      } catch {
        case _: NoSuchElementException => x
      }
    }
