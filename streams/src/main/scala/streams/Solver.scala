...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.map { case (next, move) => (next, move :: history) }.toStream
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, move) #:: xs => {
      val newNeighbor = newNeighborsOnly(neighborsWithHistory(block, move), explored)
      (block, move) #:: from(xs ++ newNeighbor, explored + block)
    }
    case _ => Stream.empty
  }
