...
class BloxorzSuite extends FunSuite {
  ...
  test("findChar level 1 - start pos") {
    new Level1 {
      assert(startPos == Pos(1,1))
    }
  }

  test("findChar level 1 - end pos") {
    new Level1 {
      assert(goal == Pos(7,4))
    }
  }

  test("Block is legal in level 1") {
    new Level1 {
  ...
  test("Block is stand in level 1") {
    new Level1 {
  ...
  test("startBlock in level 1") {
    new Level1 {
      assert(startPos == Pos(startBlock.b1.row, startBlock.b2.col))
    }
  }

  test("neighbor with history in level 1") {
    new Level1 {
      assert(
  ...
  test("new neighbor only in level 1") {
    new Level1 {
      assert(
        newNeighborsOnly(
          Set(
